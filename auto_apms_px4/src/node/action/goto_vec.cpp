...
public:
  ...
  bool setGoal(Goal & goal)
  {
    if (auto any_locked = getLockedPortContent(INPUT_KEY_VEC)) {
      if (any_locked->empty()) {
        RCLCPP_ERROR(
          logger_, "%s - Value at blackboard entry {%s} is empty", context_.getFullyQualifiedTreeNodeName(this).c_str(),
          INPUT_KEY_VEC);
        return false;
        ...
        RCLCPP_ERROR(
          logger_, "%s - Failed to cast pointer {%s}", context_.getFullyQualifiedTreeNodeName(this).c_str(),
          INPUT_KEY_VEC);
        return false;
      }
    }
...
AUTO_APMS_BEHAVIOR_TREE_DECLARE_NODE(auto_apms_px4::GoToVectorAction)
