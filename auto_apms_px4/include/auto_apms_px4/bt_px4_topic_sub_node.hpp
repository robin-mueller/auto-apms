...
/**
 * Currently, BehaviorTree.ROS2 does not support changing the QoS settings, so we have to write our own class for
...
class PX4RosTopicSubNode : public BT::ConditionNode
{
   ...
   protected:
    ...
    rclcpp::Logger logger()
    {
        ...
        return rclcpp::get_logger("PX4RosTopicSubNode");
    }
   ...
   public:
    ...
    virtual ~PX4RosTopicSubNode() { signal_connection_.disconnect(); }
...
//----------------------------------------------------------------
template <class T>
inline PX4RosTopicSubNode<T>::SubscriberInstance::SubscriberInstance(std::shared_ptr<rclcpp::Node> node,
                                                                     const std::string& topic_name)
{
    ...
    // to
    // subscriber = node->create_subscription<T>(topic_name, rclcpp::QoS(1).best_effort(), callback, option);
    subscriber = node->create_subscription<T>(topic_name, rclcpp::QoS(1).best_effort(), callback, option);
}
        ...
        if (bb_topic_name.empty() || bb_topic_name == "__default__placeholder__") {
            if (params.default_port_value.empty()) {
                throw std::logic_error(
                    "Both [topic_name] in the InputPort and the RosNodeParams "
                    "are empty.");
            }
            else {
...
inline bool PX4RosTopicSubNode<T>::createSubscriber(const std::string& topic_name)
{
    ...
    if (sub_instance_->last_msg) { last_msg_ = sub_instance_->last_msg; }
...
inline NodeStatus PX4RosTopicSubNode<T>::tick()
{
    ...
    if (!sub_instance_) { createSubscriber(topic_name); }
    ...
    if (!latchLastMessage()) { last_msg_.reset(); }
    return status;
}
