...
#include "auto_apms_interfaces/action/task_test_action.hpp"
...
class TestTask : public Task<auto_apms_interfaces::action::TaskTestAction>
{
public:
  using ActionType = auto_apms_interfaces::action::TaskTestAction;
...
protected:
  TestFixture()
  {
    ...
    task_client = std::make_shared<ActionClientWrapper<TestTask::ActionType>>(*node, task_name);
  }
  ...
  TestTask::Goal goal;
  std::shared_ptr<ActionClientWrapper<TestTask::ActionType>> task_client;
};
...
{
  goal.request = 4;
  ActionClientWrapper<TestTask::ActionType>::SendGoalOptions send_goal_options;
  send_goal_options.feedback_callback =
      [this](ActionClientWrapper<TestTask::ActionType>::ClientGoalHandle::SharedPtr,
             const std::shared_ptr<const ActionClientWrapper<TestTask::ActionType>::Feedback>) {
