    ...
    execute = ExecuteProcess(
        cmd=[
            [
                ...
                f" run auto_apms upload_tree '{NAMESPACE}' '{executor_name}' '{package_name}' '{filename}'",
            ]
        ],
        name=process_name,
        shell=True,
...
def generate_launch_description():
    bt_executor_launch = IncludeLaunchDescription(
        launch_description_source=PythonLaunchDescriptionSource(
            [
                os.path.join(
                    get_package_share_directory("auto_apms"),
                    "launch",
                    "maneuver_launch.py",
                ),
            ]
    ...
    container = ComposableNodeContainer(
        ...
        composable_node_descriptions=[
            ComposableNode(
                package="auto_apms",
                namespace=NAMESPACE,
                plugin="auto_apms::FlightOrchestratorExecutor",
                parameters=[{"state_change_logging": False}],
            ),
            ComposableNode(
                package="auto_apms",
                namespace=NAMESPACE,
                plugin="auto_apms::SafetyMonitorExecutor",
                parameters=[{"state_change_logging": False}],
            ),
            ComposableNode(
                package="auto_apms",
                namespace=NAMESPACE,
                plugin="auto_apms::MissionManagerExecutor",
                parameters=[{"state_change_logging": False}],
            ),
            ComposableNode(
                package="auto_apms",
                namespace=NAMESPACE,
                plugin="auto_apms::ContingencyManagerExecutor",
                parameters=[{"state_change_logging": False}],
            ),
        ],
        output="screen",
    ...
    upload_orchestrator_tree = upload_tree_action("flight_orchestrator", "auto_apms", "flight_orchestrator")
