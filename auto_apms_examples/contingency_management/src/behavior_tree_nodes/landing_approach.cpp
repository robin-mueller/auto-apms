...
public:
  ...
  BT::NodeStatus onTick(const std::shared_ptr<LandingApproachMsg> & last_msg_ptr) override final
  {
    ...
    if (!last_msg_ptr) {
      RCLCPP_WARN(context_.getLogger(), "%s - No new landing approach message was received", name().c_str());
...
AUTO_APMS_BEHAVIOR_TREE_DECLARE_NODE(auto_apms::ops_engine::IsApproachingLanding)
