...
inline BT::NodeStatus RosServiceNode<T>::tick()
{
    ...
    if (!srv_instance_ || (status() == BT::NodeStatus::IDLE && service_name_should_be_checked_)) {
        ...
        if (service_name_ != service_name) { createClient(service_name); }
    }
    ...
    if (status() == BT::NodeStatus::IDLE) {
        ...
        if (!srv_instance_->service_client->service_is_ready()) { return onFailure(SERVICE_UNREACHABLE); }
    ...
    if (status() == BT::NodeStatus::RUNNING) {
        ...
        if (!response_received_) {
            ...
            if (ret != rclcpp::FutureReturnCode::SUCCESS) {
                if ((now() - time_request_sent_) > timeout) { return CheckStatus(onFailure(SERVICE_TIMEOUT)); }
                else {
                    return BT::NodeStatus::RUNNING;
                }
            }
                ...
                if (!response_) { throw exceptions::RosNodeError("Request was rejected by the service"); }
            }
        }
...
{
    if (status() == BT::NodeStatus::RUNNING) { resetStatus(); }
}
