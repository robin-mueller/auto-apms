...
enum class BTExecutorState : uint8_t
{
  IDLE,
  RUNNING,
  PAUSED,
...
enum class BTExecutorCommand : uint8_t
{
  RUN,
  PAUSE,
  HALT,
...
protected:
  ...
  enum class ClosureConduct : uint8_t
  {
    SUCCEED,
    ABORT,
    RESTART
  ...
  BTExecutor(const std::string& name, const rclcpp::NodeOptions& options, const int groot2_server_port = 5555,
...
private:
  ...
  LaunchHandleCancel(std::shared_ptr<rclcpp_action::ServerGoalHandle<LaunchExecutorAction>> goal_handle_ptr);
  ...
  rclcpp_action::CancelResponse
  CommandHandleCancel(std::shared_ptr<rclcpp_action::ServerGoalHandle<CommandAction>> goal_handle_ptr);
