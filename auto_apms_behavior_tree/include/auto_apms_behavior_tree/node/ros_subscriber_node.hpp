...
class RosSubscriberNode : public BT::ConditionNode
{
   public:
     ...
     * */
    explicit RosSubscriberNode(const std::string& instance_name,
...
template <class T>
inline RosSubscriberNode<T>::RosSubscriberNode(const std::string& instance_name,
                                               const BT::NodeConfig& conf,
                                               const RosNodeParams& params,
...
{
    if (topic_name.empty()) { throw exceptions::RosNodeError("topic_name is empty"); }
    if (sub_instance_) { throw exceptions::RosNodeError("Can't call createSubscriber more than once"); }
