...
class RosPublisherNode : public BT::ConditionNode
{
   public:
     ...
     * */
    explicit RosPublisherNode(const std::string& instance_name,
...
template <class T>
inline RosPublisherNode<T>::RosPublisherNode(const std::string& instance_name,
                                             const BT::NodeConfig& conf,
                                             const RosNodeParams& params,
...
inline BT::NodeStatus RosPublisherNode<T>::tick()
{
    ...
    if (!publisher_ || (status() == BT::NodeStatus::IDLE && topic_name_may_change_)) {
        ...
        if (prev_topic_name_ != topic_name) { createPublisher(topic_name); }
    }

    T msg;
    if (!setMessage(msg)) { return BT::NodeStatus::FAILURE; }
