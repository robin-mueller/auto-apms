...
BTExecutor::BTExecutor(const std::string& name, const rclcpp::NodeOptions& options, const int groot2_server_port,
  ...
  , bt_tick_interval_{ bt_tick_interval }
{
  ...
  auto upload_service_name = name + std::string(BT_EXECUTOR_UPLOAD_TREE_SERVICE_NAME_SUFFIX);
  auto launch_action_name = name + std::string(BT_EXECUTOR_RUN_ACTION_NAME_SUFFIX);
  ...
  launch_action_ptr_ = rclcpp_action::create_server<LaunchExecutorAction>(
      node_ptr_, launch_action_name, std::bind(&BTExecutor::LaunchHandleGoal, this, _1, _2),
      std::bind(&BTExecutor::LaunchHandleCancel, this, _1), std::bind(&BTExecutor::LaunchHandleAccepted, this, _1));

  upload_service_ptr_ = node_ptr_->create_service<UploadBehaviorTreeService>(
      upload_service_name, std::bind(&BTExecutor::UploadBehaviorTree, this, _1, _2));

  command_action_ptr_ = rclcpp_action::create_server<CommandAction>(
      node_ptr_, command_action_name, std::bind(&BTExecutor::CommandHandleGoal, this, _1, _2),
      std::bind(&BTExecutor::CommandHandleCancel, this, _1), std::bind(&BTExecutor::CommandHandleAccepted, this, _1));
...
BTExecutor::Command BTExecutor::ReviewControlCommand(Command current_command, State)
{
...
BTExecutor::ClosureConduct BTExecutor::OnResult(bool)
{
  return ClosureConduct::SUCCEED;
}
                                                         ...
                                                         std::shared_ptr<const LaunchExecutorAction::Goal> goal_ptr)
{
  ...
  {
    RCLCPP_WARN(node_ptr_->get_logger(), "Goal %s was REJECTED because a tree with ID '%s' is already executing",
                rclcpp_action::to_string(uuid).c_str(), GetCreatedTreeID().c_str());
    return rclcpp_action::GoalResponse::REJECT;
  }

  if (!behavior_tree_ptr_)
  {
    RCLCPP_WARN(node_ptr_->get_logger(), "Goal %s was REJECTED because no tree has been created yet",
...
BTExecutor::LaunchHandleCancel(std::shared_ptr<rclcpp_action::ServerGoalHandle<LaunchExecutorAction>> goal_handle_ptr)
{
  ...
  execution_timer_ptr_ = node_ptr_->create_wall_timer(
      ...
      [this, goal_handle_ptr, action_result_ptr = std::make_shared<LaunchExecutorAction::Result>()]() {
        ...
        if (!running_action_history.empty())
        {
          ...
          std::ostringstream oss;
          std::copy(running_action_history.begin(), running_action_history.end() - 1,
        ...
        if (execution_state_before == State::IDLE)
        {
          BeforeFirstTick(*global_blackboard_ptr_);
        }
...
rclcpp_action::CancelResponse
BTExecutor::CommandHandleCancel(std::shared_ptr<rclcpp_action::ServerGoalHandle<CommandAction>> goal_handle_ptr)
{
  (void)goal_handle_ptr;
  return rclcpp_action::CancelResponse::ACCEPT;
}
...
void BTExecutor::CommandHandleAccepted(std::shared_ptr<rclcpp_action::ServerGoalHandle<CommandAction>> goal_handle_ptr)
{
  ...
  command_request_timer_ptr_ = node_ptr_->create_wall_timer(
      ...
      [this, goal_handle_ptr, prev_command, action_result_ptr = std::make_shared<CommandAction::Result>()]() {
        ...
        {
          // We don't know how far the process proceeded, but the least we can do is reset the control command to
          // its previous state
...
void BTExecutor::CreateTree(const std::string& tree_id)
{
  ...
  if (!bt_factory_ptr_)
  {
    throw std::runtime_error("Tree factory hasn't been set up yet");
  }

  auto new_tree =
      std::make_unique<BT::Tree>(bt_factory_ptr_->createTree(tree_id, BT::Blackboard::create(global_blackboard_ptr_)));
...
rclcpp::Node::SharedPtr BTExecutor::node()
{
  return node_ptr_;
}
BT::Blackboard::Ptr BTExecutor::global_blackboard()
{
