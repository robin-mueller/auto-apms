...
TreeConstructor TreeExecutorNode::makeTreeConstructor(
  const std::string & build_handler_request, const std::string & entrypoint, const core::NodeManifest & node_manifest,
  const core::NodeManifest & node_overrides)
{
  // Request the tree identity
  if (build_handler_ptr_ && !build_handler_ptr_->setBuildRequest(build_handler_request, entrypoint, node_manifest)) {
  ...
  return [this, node_manifest, node_overrides](TreeBlackboardSharedPtr bb_ptr) {
  ...
  const rclcpp_action::GoalUUID & uuid, std::shared_ptr<const StartActionContext::Goal> goal_ptr)
{
  ...
  try {
    tree_constructor_ =
      makeTreeConstructor(goal_ptr->build_request, goal_ptr->entrypoint, node_manifest, node_overrides);
  } catch (const std::exception & e) {
