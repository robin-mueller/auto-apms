...
BTExecutorClient::BTExecutorClient(rclcpp::Node& node, const std::string& executor_name)
  ...
  , upload_service_name_{ executor_name + std::string(BT_EXECUTOR_UPLOAD_TREE_SERVICE_NAME_SUFFIX) }
  , launch_executor_action_name_{ executor_name + std::string(BT_EXECUTOR_RUN_ACTION_NAME_SUFFIX) }
{
...
bool BTExecutorClient::UploadBehaviorTree(std::string xml_data, const std::string& main_tree_id)
{
  ...
  auto new_end =
      std::unique(xml_data.begin(), xml_data.end(), [](char a, char b) { return std::isspace(a) && std::isspace(b); });
  xml_data.erase(new_end, xml_data.end());

  if (!upload_client_ptr_->wait_for_service(WAIT_FOR_SERVER_TIMEOUT))
  {
    RCLCPP_ERROR(logger_, "UploadBehaviorTree: Registration service '%s' is not available",
...
std::shared_future<BTExecutorClient::ExecutionResultSharedPtr> BTExecutorClient::RequestLaunch()
{
  ...
  {
    RCLCPP_ERROR(logger_, "RequestLaunch: Launch action '%s' is not available", launch_executor_action_name_.c_str());
      ...
      [this, promise_ptr](const rclcpp_action::ClientGoalHandle<LaunchExecutorAction>::WrappedResult& wr) {
        switch (wr.code)
        {
          ...
          case rclcpp_action::ResultCode::ABORTED:
            RCLCPP_WARN(logger_, "Behavior tree '%s' was aborted: %s", root_tree_id_.c_str(),
  ...
  switch (
      rclcpp::spin_until_future_complete(node_base_interface_ptr_, goal_response_future, LAUNCH_GOAL_RESPONSE_TIMEOUT))
  {
    case rclcpp::FutureReturnCode::SUCCESS:
      break;
    case rclcpp::FutureReturnCode::TIMEOUT:
...
bool BTExecutorClient::RequestCancelation()
{
  ...
  switch (rclcpp::spin_until_future_complete(node_base_interface_ptr_, cancel_response_future, CANCEL_RESPONSE_TIMEOUT))
  {
    case rclcpp::FutureReturnCode::SUCCESS:
      break;
    case rclcpp::FutureReturnCode::TIMEOUT:
...
bool BTExecutorClient::has_launched()
{
  return !!launch_executor_goal_handle_;
}

const std::string& BTExecutorClient::root_tree_id()
{
  return root_tree_id_;
}

const std::string& BTExecutorClient::running_tree_action_name()
{
  return running_tree_action_name_;
}

double BTExecutorClient::running_tree_action_timestamp()
{
  return running_tree_action_timestamp_;
}
