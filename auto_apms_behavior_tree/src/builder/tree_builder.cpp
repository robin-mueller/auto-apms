...
TreeBuilder & TreeBuilder::loadNodePlugins(
  rclcpp::Node::SharedPtr node_ptr, const NodeManifest & node_manifest, NodeRegistrationClassLoader & tree_node_loader,
  bool override)
{
  ...
  for (const auto & [node_name, params] : node_manifest.getInternalMap()) {
    ...
    RCLCPP_DEBUG(
      ...
      params.class_name.c_str(), tree_node_loader.getClassLibraryPath(params.class_name).c_str());
...
std::unordered_map<std::string, BT::NodeType> TreeBuilder::getRegisteredNodes() const
{
  ...
  for (const auto & it : factory_ptr_->manifests()) registered_nodes.insert({it.first, it.second.type});
  return registered_nodes;
}

TreeBuilder & TreeBuilder::loadNodePlugins(
  rclcpp::Node::SharedPtr node_ptr, const NodeManifest & node_manifest, bool override)
{
...
TreeBuilder & TreeBuilder::mergeTreesFromDocument(const tinyxml2::XMLDocument & doc)
{
  ...
  if (!child) {
    throw exceptions::TreeBuildError(
      "Cannot merge trees: Other document has no children named '" + std::string(TREE_ELEMENT_NAME) + "'.");
  }
...
bool TreeBuilder::isExistingTreeName(const std::string & tree_name)
{
  ...
  if (std::find(all_names.begin(), all_names.end(), tree_name) == all_names.end()) return false;
  return true;
}
...
tinyxml2::XMLElement * TreeBuilder::insertNewTreeElement(const std::string & tree_name)
{
  if (isExistingTreeName(tree_name)) {
    throw exceptions::TreeBuildError(
      "Cannot insert a tree element with name '" + tree_name + "' because it already existing.");
  }
