...
def collect_plugin_xml_paths(exclude_packages: set[str] = None) -> list[tuple[str, str]]:
    """
    ...
    for p in packages:
        xml_paths.append((p, get_plugin_xml_path(p)))
    return xml_paths


def get_plugin_names_with_base_type(
    base_class_type: str, exclude_packages: set[str] = None, *, per_package=False
    ...
    """
    plugins_per_package = defaultdict(list)
    for package, xml_path in collect_plugin_xml_paths(exclude_packages):
        try:
            ...
            for library in root.findall("library"):
                for class_elem in library.findall("class"):
                    if class_elem.get("base_class_type") == base_class_type:
                        ...
                        if plugin_name:
                            plugins_per_package[package].append(plugin_name)
    ...
    return (
        plugins_per_package
        if per_package
        else [plugin_name for plugins in plugins_per_package.values() for plugin_name in plugins]
    )
