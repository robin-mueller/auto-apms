        ...
        for (const auto& [node_name, params] : manifest.map()) {
            ...
            if (std::find(classes.begin(), classes.end(), factory_classname) == classes.end()) {
                throw std::runtime_error{"Node '" + node_name + " (" + params.class_name +
                                         ...
                                         "auto_apms_register_behavior_tree_nodes() or forgot to call the "
                                         "AUTO_APMS_REGISTER_BEHAVIOR_TREE_NODE "
            ...
            try {
                const auto plugin_instance =
                    class_loader.createUniqueInstance<detail::BTNodePluginBase>(factory_classname);
